 ...
 * Fixed a bug where Surrogate was not rendered in standalone
 ...
 /* ---- v 3.1.2----
	...
	* Corrections in volumetric shadows integration in the lighting model
	 ...
	 * A textured Fog Volume can be now excluded from the low-res process. Useful to render FV with a different camera, e.g water refraction
    ...
    public void RenderSurrogate()
    {
        ...
        if (IsVisible)
        {
            ...
            if (GameCameraGO.GetComponent<FogVolumeRenderer>()._Downsample > 0
                    ...
                    && CreateSurrogate && mesh != null && _FogType == FogVolume.FogType.Textured)
            {
