    ...
    public override void OnInspectorGUI()
    {
        ...
        if (_target._FogType == FogVolume.FogType.Textured)
        {
            ...
            if (_target.showVolumeProperties)
            {
                ...
                _target._OptimizationFactor = EditorGUILayout.Slider(VariableField("Optimization Factor", "Used to increase the distance between iterations so that we can have more quality at close distances. Far distances will be less opaque"), _target._OptimizationFactor, 0, .000001f);
            ...
            if (_target.showOtherOptions)
            {
              ...
              //  if(!_target.CreateSurrogate)
                ...
                if (_target.GameCameraGO != null /*&& _target._FogType == FogVolume.FogType.Textured*/)
                    if (_target.GameCameraGO.GetComponent<FogVolumeRenderer>()._Downsample > 0 && _target._FogType == FogVolume.FogType.Textured && _target.GameCameraGO.GetComponent<FogVolumeRenderer>())
        ...
        if (_target.generalInfo)
        {
            ...
            EditorGUILayout.LabelField("Version 3.2.1");
            EditorGUILayout.LabelField("Release date: August 2017");
